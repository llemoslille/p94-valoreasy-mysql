...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[ORDENS SERVIÇO] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[ORDENS SERVIÇO] Erro ao salvar no GCS: {e}")
        return None


def coletar_ordens_servico(connection):
    """
    Coleta os dados da tabela omie_ordens_servico do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados ordens de serviço ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_ordens_servico"
            ...
            print(f"[ORDENS SERVIÇO] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[ORDENS SERVIÇO] Erro ao coletar dados: {e}")
        return None


def processar_ordens_servico(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_ordens_servico(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "ordens_servico.parquet")

            return resultado is not None
        else:
            print(f"[ORDENS SERVIÇO] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[ORDENS SERVIÇO] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_ordens_servico(connection, config_yaml)
