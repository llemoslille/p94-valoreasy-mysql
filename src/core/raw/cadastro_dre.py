...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[CADASTRO DRE] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[CADASTRO DRE] Erro ao salvar no GCS: {e}")
        return None


def coletar_cadastro_dre(connection):
    """
    Coleta os dados da tabela omie_cadastro_dre do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados cadastro DRE ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_cadastro_dre"
            ...
            print(f"[CADASTRO DRE] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[CADASTRO DRE] Erro ao coletar dados: {e}")
        return None


def processar_cadastro_dre(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_cadastro_dre(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "cadastro_dre.parquet")

            return resultado is not None
        else:
            print(f"[CADASTRO DRE] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[CADASTRO DRE] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_cadastro_dre(connection, config_yaml)
