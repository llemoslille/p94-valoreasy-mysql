...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[CONTAS A PAGAR] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[CONTAS A PAGAR] Erro ao salvar no GCS: {e}")
        return None


def coletar_contas_a_pagar(connection):
    """
    Coleta os dados da tabela omie_contas_a_pagar do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados contas a pagar ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_contas_a_pagar"
            ...
            print(f"[CONTAS A PAGAR] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[CONTAS A PAGAR] Erro ao coletar dados: {e}")
        return None


def processar_contas_a_pagar(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_contas_a_pagar(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "contas_a_pagar.parquet")

            return resultado is not None
        else:
            print(f"[CONTAS A PAGAR] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[CONTAS A PAGAR] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_contas_a_pagar(connection, config_yaml)
