...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[TIPO FAT CONTRATO] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[TIPO FAT CONTRATO] Erro ao salvar no GCS: {e}")
        return None


def coletar_tipo_fat_contrato(connection):
    """
    Coleta os dados da tabela omie_tipo_fat_contrato do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados tipos de faturamento de contrato ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_tipo_fat_contrato"
            ...
            print(f"[TIPO FAT CONTRATO] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[TIPO FAT CONTRATO] Erro ao coletar dados: {e}")
        return None


def processar_tipo_fat_contrato(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_tipo_fat_contrato(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "tipo_fat_contrato.parquet")

            return resultado is not None
        else:
            print(f"[TIPO FAT CONTRATO] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[TIPO FAT CONTRATO] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_tipo_fat_contrato(connection, config_yaml)
