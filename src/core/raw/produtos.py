...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[PRODUTOS] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[PRODUTOS] Erro ao salvar no GCS: {e}")
        return None


def coletar_produtos(connection):
    """
    Coleta os dados da tabela omie_produtos do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados produtos ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_produtos"
            ...
            print(f"[PRODUTOS] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[PRODUTOS] Erro ao coletar dados: {e}")
        return None


def processar_produtos(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_produtos(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "produtos.parquet")

            return resultado is not None
        else:
            print(f"[PRODUTOS] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[PRODUTOS] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_produtos(connection, config_yaml)
