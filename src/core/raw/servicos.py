...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[SERVIÇOS] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[SERVIÇOS] Erro ao salvar no GCS: {e}")
        return None


def coletar_servicos(connection):
    """
    Coleta os dados da tabela omie_servicos do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados serviços ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_servicos"
            ...
            print(f"[SERVIÇOS] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[SERVIÇOS] Erro ao coletar dados: {e}")
        return None


def processar_servicos(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_servicos(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "servicos.parquet")

            return resultado is not None
        else:
            print(f"[SERVIÇOS] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[SERVIÇOS] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_servicos(connection, config_yaml)
