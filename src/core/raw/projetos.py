...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[PROJETOS] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[PROJETOS] Erro ao salvar no GCS: {e}")
        return None


def coletar_projetos(connection):
    """
    Coleta os dados da tabela omie_projetos do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados projetos ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_projetos"
            ...
            print(f"[PROJETOS] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[PROJETOS] Erro ao coletar dados: {e}")
        return None


def processar_projetos(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_projetos(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "projetos.parquet")

            return resultado is not None
        else:
            print(f"[PROJETOS] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[PROJETOS] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_projetos(connection, config_yaml)
