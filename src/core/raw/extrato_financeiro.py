...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[EXTRATO FINANCEIRO] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[EXTRATO FINANCEIRO] Erro ao salvar no GCS: {e}")
        return None


def coletar_extrato_financeiro(connection):
    """
    Coleta os dados da tabela omie_extrato_financeiro do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados extrato financeiro ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_extrato_financeiro"
            ...
            print(f"[EXTRATO FINANCEIRO] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[EXTRATO FINANCEIRO] Erro ao coletar dados: {e}")
        return None


def processar_extrato_financeiro(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_extrato_financeiro(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "extrato_financeiro.parquet")

            return resultado is not None
        else:
            print(f"[EXTRATO FINANCEIRO] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[EXTRATO FINANCEIRO] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_extrato_financeiro(connection, config_yaml)
