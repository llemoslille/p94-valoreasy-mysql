...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[CONTRATOS] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[CONTRATOS] Erro ao salvar no GCS: {e}")
        return None


def coletar_contratos(connection):
    """
    Coleta os dados da tabela omie_contratos do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados contratos ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_contratos"
            ...
            print(f"[CONTRATOS] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[CONTRATOS] Erro ao coletar dados: {e}")
        return None


def processar_contratos(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_contratos(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "contratos.parquet")

            return resultado is not None
        else:
            print(f"[CONTRATOS] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[CONTRATOS] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_contratos(connection, config_yaml)
