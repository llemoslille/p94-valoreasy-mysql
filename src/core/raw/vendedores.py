...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(f"[VENDEDORES] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[VENDEDORES] Erro ao salvar no GCS: {e}")
        return None


def coletar_vendedores(connection):
    """
    Coleta os dados da tabela omie_vendedores do banco de dados MySQL.

    Args:
        connection: Conexão ativa com o banco de dados MySQL

    Returns:
        pd.DataFrame: DataFrame com os dados vendedores ou None em caso de erro
    """
    try:
        if connection.is_connected():
            # Criar cursor e executar consulta
            cursor = connection.cursor()
            query = "SELECT * FROM omie_vendedores"
            ...
            print(f"[VENDEDORES] Total de registros coletados: {len(df)}")

            cursor.close()
            return df

    except Error as e:
        print(f"[VENDEDORES] Erro ao coletar dados: {e}")
        return None


def processar_vendedores(connection, config_yaml):
    """
    ...
    try:
        # Coletar dados
        df = coletar_vendedores(connection)
        ...
        if df is not None and len(df) > 0:
            ...
            resultado = salvar_no_gcs(
                df, bucket_name, caminho_gcs, credentials_path, "vendedores.parquet")

            return resultado is not None
        else:
            print(f"[VENDEDORES] Nenhum dado coletado.")
            return False

    except Exception as e:
        print(f"[VENDEDORES] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sucesso = processar_vendedores(connection, config_yaml)
