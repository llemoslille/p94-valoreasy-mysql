...
def ler_do_gcs(bucket_name, caminho_gcs, credentials_path):
    ...
    try:
        ...
        print(
            f"[CONTAS A RECEBER SILVER] Arquivo lido do GCS: gs://{bucket_name}/{caminho_gcs}")
        print(f"[CONTAS A RECEBER SILVER] Total de registros lidos: {len(df)}")
        ...
        print(f"[CONTAS A RECEBER SILVER] Erro ao ler do GCS: {e}")
        return None
...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(
            f"[CONTAS A RECEBER SILVER] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[CONTAS A RECEBER SILVER] Erro ao salvar no GCS: {e}")
        return None
...
def normalizar_categorias(df):
    ...
    try:
        print(
            f"[CONTAS A RECEBER SILVER] Iniciando normalização da coluna categorias...")

        # Verificar se a coluna existe
        if 'categorias' not in df.columns:
            print(
                f"[CONTAS A RECEBER SILVER] AVISO: Coluna 'categorias' não encontrada no DataFrame")
            return df
        ...
        print(
            f"[CONTAS A RECEBER SILVER] Registros com categorias válidas: {dados_validos}/{len(df_normalizado)}")

        if dados_validos == 0:
            print(
                f"[CONTAS A RECEBER SILVER] Nenhum dado válido na coluna categorias para normalizar")
        ...
        print(
            f"[CONTAS A RECEBER SILVER] Total de categorias a expandir: {total_categorias}")
        ...
        print(f"[CONTAS A RECEBER SILVER] Normalização concluída!")
        print(
            f"[CONTAS A RECEBER SILVER] Registros antes: {len(df_normalizado)}")
        print(
            f"[CONTAS A RECEBER SILVER] Registros depois: {len(df_exploded)}")
        if not categorias_df.empty:
            print(
                f"[CONTAS A RECEBER SILVER] Colunas de categoria criadas: {list(categorias_df.columns)}")

        return df_exploded

    except Exception as e:
        print(f"[CONTAS A RECEBER SILVER] Erro ao normalizar categorias: {e}")
        import traceback
        print(
            f"[CONTAS A RECEBER SILVER] Detalhes do erro:\n{traceback.format_exc()}")
        return df


def processar_contas_a_receber_silver(config_yaml):
    """
    ...
    try:
        ...
        caminho_bronze = f"{config_yaml['gcs']['bronze']['folder']}/mysql_omie/contas_a_receber.parquet"
        credentials_path = config_yaml['credentials-path']

        # Ler dados da camada bronze
        print(f"[CONTAS A RECEBER SILVER] Lendo dados da camada bronze...")
        df = ler_do_gcs(bucket_name, caminho_bronze, credentials_path)

        if df is None or len(df) == 0:
            print(
                f"[CONTAS A RECEBER SILVER] Nenhum dado encontrado na camada bronze.")
            return False
        ...
        resultado = salvar_no_gcs(
            ...
            caminho_silver,
            credentials_path,
            "contas_a_receber.parquet"
        )

        return resultado is not None

    except Exception as e:
        print(f"[CONTAS A RECEBER SILVER] Erro no processamento: {e}")
        return False
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("PROCESSAMENTO SILVER - CONTAS A RECEBER")
    print("="*80)

    config_yaml = carregar_config()

    try:
        sucesso = processar_contas_a_receber_silver(config_yaml)
        print(f"\n{'='*80}")
