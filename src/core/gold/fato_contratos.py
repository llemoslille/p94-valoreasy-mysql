...
def ler_do_gcs(bucket_name, caminho_gcs, credentials_path):
    ...
    try:
        ...
        print(
            f"[CONTRATOS GOLD] Arquivo lido do GCS: gs://{bucket_name}/{caminho_gcs}")
        print(f"[CONTRATOS GOLD] Total de registros lidos: {len(df)}")
        ...
        print(f"[CONTRATOS GOLD] Erro ao ler do GCS: {e}")
        return None
...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(
            f"[CONTRATOS GOLD] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[CONTRATOS GOLD] Erro ao salvar no GCS: {e}")
        return None


def normalizar_nomes_colunas(df):
    """
    Normaliza os nomes das colunas substituindo pontos por underscores.
    Isso é necessário porque o BigQuery não suporta pontos em nomes de colunas
    ao ler arquivos Parquet.

    Args:
        df: DataFrame com colunas a serem normalizadas

    Returns:
        pd.DataFrame: DataFrame com nomes de colunas normalizados
    """
    try:
        print(f"[CONTRATOS GOLD] Normalizando nomes de colunas...")

        # Criar dicionário de mapeamento: coluna antiga -> coluna nova
        colunas_antigas = list(df.columns)
        colunas_novas = [col.replace('.', '_') for col in colunas_antigas]

        # Verificar se houve mudanças
        colunas_alteradas = [antiga for antiga, nova in zip(
            colunas_antigas, colunas_novas) if antiga != nova]

        if colunas_alteradas:
            print(
                f"[CONTRATOS GOLD] Colunas que serão renomeadas: {len(colunas_alteradas)}")
            for antiga, nova in zip(colunas_antigas, colunas_novas):
                if antiga != nova:
                    print(f"[CONTRATOS GOLD]   '{antiga}' -> '{nova}'")

            # Renomear colunas
            df = df.rename(columns=dict(zip(colunas_antigas, colunas_novas)))
            print(f"[CONTRATOS GOLD] Normalização de nomes de colunas concluída!")
        else:
            print(f"[CONTRATOS GOLD] Nenhuma coluna precisa ser renomeada.")

        return df

    except Exception as e:
        print(f"[CONTRATOS GOLD] Erro ao normalizar nomes de colunas: {e}")
        import traceback
        print(f"[CONTRATOS GOLD] Detalhes do erro:\n{traceback.format_exc()}")
        return df


def processar_fato_contratos_gold(config_yaml):
    """
    ...
    try:
        ...
        caminho_silver = f"{config_yaml['gcs']['silver']['folder']}/mysql_omie/contratos.parquet"
        credentials_path = config_yaml['credentials-path']

        # Ler dados da camada silver
        print(f"[CONTRATOS GOLD] Lendo dados da camada silver...")
        df = ler_do_gcs(bucket_name, caminho_silver, credentials_path)

        if df is None or len(df) == 0:
            print(f"[CONTRATOS GOLD] Nenhum dado encontrado na camada silver.")
            return False

        print(
            f"[CONTRATOS GOLD] Total de registros para processar: {len(df)}")
        print(f"[CONTRATOS GOLD] Colunas originais: {list(df.columns)}")

        # Normalizar nomes de colunas (substituir pontos por underscores)
        df = normalizar_nomes_colunas(df)
        print(
            f"[CONTRATOS GOLD] Colunas após normalização: {list(df.columns)}")
        ...
        if colunas_categoria_numericas:
            print(
                f"[CONTRATOS GOLD] Convertendo {len(colunas_categoria_numericas)} colunas categoria_* numéricas para string: {colunas_categoria_numericas}")
        ...
        if colunas_codigo_numericas:
            print(
                f"[CONTRATOS GOLD] Convertendo {len(colunas_codigo_numericas)} colunas codigo_* numéricas para string: {colunas_codigo_numericas}")
        ...
        if len(colunas_object) > 0:
            print(
                f"[CONTRATOS GOLD] Convertendo {len(colunas_object)} colunas object para string...")
        ...
        print(f"[CONTRATOS GOLD] Erro no processamento: {e}")
        import traceback
        print(
            f"[CONTRATOS GOLD] Detalhes do erro:\n{traceback.format_exc()}")
        return False
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("PROCESSAMENTO GOLD - FATO CONTRATOS")
    print("="*80)

    config_yaml = carregar_config()

    try:
        sucesso = processar_fato_contratos_gold(config_yaml)
        print(f"\n{'='*80}")
