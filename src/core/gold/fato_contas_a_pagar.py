...
def ler_do_gcs(bucket_name, caminho_gcs, credentials_path):
    ...
    try:
        ...
        print(
            f"[CONTAS A PAGAR GOLD] Arquivo lido do GCS: gs://{bucket_name}/{caminho_gcs}")
        print(f"[CONTAS A PAGAR GOLD] Total de registros lidos: {len(df)}")
        ...
        print(f"[CONTAS A PAGAR GOLD] Erro ao ler do GCS: {e}")
        return None
...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(
            f"[CONTAS A PAGAR GOLD] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[CONTAS A PAGAR GOLD] Erro ao salvar no GCS: {e}")
        return None


def processar_fato_contas_a_pagar_gold(config_yaml):
    """
    ...
    try:
        ...
        caminho_silver = f"{config_yaml['gcs']['silver']['folder']}/mysql_omie/contas_a_pagar.parquet"
        credentials_path = config_yaml['credentials-path']

        # Ler dados da camada silver
        print(f"[CONTAS A PAGAR GOLD] Lendo dados da camada silver...")
        df = ler_do_gcs(bucket_name, caminho_silver, credentials_path)

        if df is None or len(df) == 0:
            print(f"[CONTAS A PAGAR GOLD] Nenhum dado encontrado na camada silver.")
            return False

        print(
            f"[CONTAS A PAGAR GOLD] Total de registros para processar: {len(df)}")
        print(f"[CONTAS A PAGAR GOLD] Colunas: {list(df.columns)}")
        ...
        if colunas_categoria_numericas:
            print(
                f"[CONTAS A PAGAR GOLD] Convertendo {len(colunas_categoria_numericas)} colunas categoria_* numéricas para string: {colunas_categoria_numericas}")
        ...
        if colunas_codigo_numericas:
            print(
                f"[CONTAS A PAGAR GOLD] Convertendo {len(colunas_codigo_numericas)} colunas codigo_* numéricas para string: {colunas_codigo_numericas}")
        ...
        if len(colunas_object) > 0:
            print(
                f"[CONTAS A PAGAR GOLD] Convertendo {len(colunas_object)} colunas object para string...")
        ...
        print(f"[CONTAS A PAGAR GOLD] Erro no processamento: {e}")
        import traceback
        print(
            f"[CONTAS A PAGAR GOLD] Detalhes do erro:\n{traceback.format_exc()}")
        return False
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("PROCESSAMENTO GOLD - FATO CONTAS A PAGAR")
    print("="*80)

    config_yaml = carregar_config()

    try:
        sucesso = processar_fato_contas_a_pagar_gold(config_yaml)
        print(f"\n{'='*80}")
