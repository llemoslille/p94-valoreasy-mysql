...
def ler_do_gcs(bucket_name, caminho_gcs, credentials_path):
    ...
    try:
        ...
        temp_file = os.path.join(temp_dir, 'temp_extrato_financeiro.parquet')
        ...
        print(
            f"[EXTRATO GOLD] Arquivo lido do GCS: gs://{bucket_name}/{caminho_gcs}")
        print(f"[EXTRATO GOLD] Total de registros lidos: {len(df)}")
        ...
        print(f"[EXTRATO GOLD] Erro ao ler do GCS: {e}")
        return None
...
def salvar_no_gcs(df, bucket_name, caminho_gcs, credentials_path, nome_arquivo):
    ...
    try:
        ...
        print(
            f"[EXTRATO GOLD] Arquivo salvo no GCS: gs://{bucket_name}/{caminho_completo}")
        ...
        print(f"[EXTRATO GOLD] Erro ao salvar no GCS: {e}")
        return None


def processar_fato_extrato_gold(config_yaml):
    """
    ...
    try:
        ...
        caminho_bronze = f"{config_yaml['gcs']['bronze']['folder']}/mysql_omie/extrato_financeiro.parquet"
        credentials_path = config_yaml['credentials-path']

        # Ler dados da camada bronze
        print(f"[EXTRATO GOLD] Lendo dados da camada bronze...")
        df = ler_do_gcs(bucket_name, caminho_bronze, credentials_path)

        if df is None or len(df) == 0:
            print(f"[EXTRATO GOLD] Nenhum dado encontrado na camada bronze.")
            return False

        print(
            f"[EXTRATO GOLD] Total de registros para processar: {len(df)}")
        print(f"[EXTRATO GOLD] Colunas: {list(df.columns)}")
        ...
        # Converter TODAS as colunas numéricas para string (BigQuery espera STRING)
        colunas_numericas = df.select_dtypes(
            include=['float64', 'float32', 'int64', 'int32']).columns
        if len(colunas_numericas) > 0:
            print(
                f"[EXTRATO GOLD] Convertendo {len(colunas_numericas)} colunas numéricas para string: {list(colunas_numericas)}")
        ...
        if len(colunas_object) > 0:
            print(
                f"[EXTRATO GOLD] Convertendo {len(colunas_object)} colunas object para string...")
        ...
        resultado = salvar_no_gcs(
            ...
            caminho_gold,
            credentials_path,
            "fato_extrato.parquet"
        )

        return resultado is not None

    except Exception as e:
        print(f"[EXTRATO GOLD] Erro no processamento: {e}")
        import traceback
        print(
            f"[EXTRATO GOLD] Detalhes do erro:\n{traceback.format_exc()}")
        return False
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("PROCESSAMENTO GOLD - FATO EXTRATO")
    print("="*80)

    config_yaml = carregar_config()

    try:
        sucesso = processar_fato_extrato_gold(config_yaml)
        print(f"\n{'='*80}")
